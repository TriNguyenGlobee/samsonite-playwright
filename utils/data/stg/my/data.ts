export const carouselItems = [
    { href: '', hasImage: true },
    { href: 'https://ssmy.stg.samsonite-asia.com/festive-gifting-2025/', hasImage: true },
    { href: 'https://ssmy.stg.samsonite-asia.com/paralux/', hasImage: true },
    { href: 'https://ssmy.stg.samsonite-asia.com/bundle-deal/', hasImage: true },
    { href: 'https://ssmy.stg.samsonite-asia.com/lite-geo/', hasImage: true },
    { href: 'https://ssmy.stg.samsonite-asia.com/price-adjustment/', hasImage: true },
    { href: 'https://ssmy.stg.samsonite-asia.com/samsonite-newstreamlite/', hasImage: true },
    { href: 'https://ssmy.stg.samsonite-asia.com/samsonite-evoaz-retro-blue/', hasImage: true }
];

export const hightlightCategoryItems = [
    { href: 'https://ssmy.stg.samsonite-asia.com/bags/', hasImage: true, aboveText: 'Ladies Bags', underText: 'For her Convenience' },
    { href: 'https://ssmy.stg.samsonite-asia.com/luggage/', hasImage: true, aboveText: 'Luggage', underText: 'Travel with Confidence' },
    { href: 'https://ssmy.stg.samsonite-asia.com/backpacks/', hasImage: true, aboveText: 'Backpacks', underText: 'Carry Functionally' },
];

export const recommendedProductItems = [
    { buttonText: 'NEW IN', divClass: 'product-pills-1' },
    { buttonText: 'BEST SELLERS', divClass: 'product-pills-2' },
    { buttonText: 'FOR HER', divClass: 'product-pills-3' },
    { buttonText: 'FOR HIM', divClass: 'product-pills-4' }
];

export const campaignData = [
    {
        href: "https://ssmy.stg.samsonite-asia.com/samsonite-black-label-major-lite%E2%80%8B/major-lite/",
        rightSideTitleText: "Major-Lite",
        hasImage: true
    }
]

// --- New Arrivals ---
export const newArrivalsItem = [
    { text: 'Paralux', href: 'https://ssmy.stg.samsonite-asia.com/paralux/' },
    { text: 'C-Lite', href: '/new-arrivals/c-lite/' },
    { text: 'Lite-Geo', href: 'https://ssmy.stg.samsonite-asia.com/lite-geo/' },
    { text: 'Relyon', href: '/new-arrivals/relyon/' },
    { text: 'Audrina', href: '/new-arrivals/audrina/' },
    { text: 'Karissa Evo', href: '/new-arrivals/karissa-evo/' },
    { text: 'Shop all new arrivals', href: 'https://ssmy.stg.samsonite-asia.com/new-arrivals/' }
]

// --- luggage-type ---
export const typeItems = [
    { text: 'Hardside', href: '/luggage/hardside/' },
    { text: 'Softside', href: '/luggage/softside/' },
    { text: 'For Kids', href: '/luggage/kids/' },
    { text: 'Shop all luggage', href: 'https://ssmy.stg.samsonite-asia.com/luggage/' }
];

// --- luggage-size ---
export const sizeItems = [
    { text: 'Cabin/Small', href: 'https://ssmy.stg.samsonite-asia.com/luggage/cabin' },
    { text: 'Medium', href: 'https://ssmy.stg.samsonite-asia.com/luggage/medium' },
    { text: 'Large', href: 'https://ssmy.stg.samsonite-asia.com/luggage/large' }
];

// --- luggage-color ---
export const colorItems = [
    {
        text: 'Mono',
        href: 'https://ssmy.stg.samsonite-asia.com/luggage/black_grey_silver_white'
    },
    {
        text: 'Cool',
        href: 'https://ssmy.stg.samsonite-asia.com/luggage/blue_green_navy'
    },
    {
        text: 'Warm',
        href: 'https://ssmy.stg.samsonite-asia.com/luggage/beige_orange_pink_red_yellow'
    },
    {
        text: 'Special',
        href: '/luggage/special/'
    },
    {
        text: 'Shop all colours',
        href: '/luggage/luggage-all-color/'
    }
];

// --- luggage-smart-feature ---
export const smartFeatureItems = [
    {
        text: 'USB Port',
        href: '/luggage/usb-port/'
    },
    {
        text: 'Suspension Wheels',
        href: '/luggage/suspension-wheels/'
    },
    {
        text: 'Double Coil Zippers',
        href: '/luggage/features/double-coil-zippers/'
    },
    {
        text: 'Easy Brake System',
        href: '/luggage/features/easy-brake-system/'
    }
];

// --- luggage-brand ---
export const brandItems = [
    {
        text: 'Samsonite',
        href: '/luggage/brand-samsonite/'
    },
    {
        text: 'Samsonite Black',
        href: '/luggage/brand-samsonite-black/'
    },
    {
        text: 'Samsonite Red',
        href: '/luggage/brand-samsonite-red/'
    }
];

// --- luggage-destination ---
export const destinationItems = [
    {
        text: 'City',
        href: '/luggage/city/'
    },
    {
        text: 'Beach',
        href: '/luggage/beach/'
    },
    {
        text: 'Adventure',
        href: '/luggage/adventure/'
    },
    {
        text: 'Business',
        href: '/luggage/business/'
    }
];

// --- luggage-collection ---
export const collectionItems = [
    {
        text: 'Minter',
        href: '/luggage/collection/minter/'
    },
    {
        text: 'Toiis C',
        href: '/luggage/collection/toiis-c/'
    },
    {
        text: 'B-Lite 4',
        href: '/luggage/collection/b-lite-4/'
    },
    {
        text: 'Azio',
        href: '/luggage/collection/azio/'
    },
    {
        text: 'Niar',
        href: '/luggage/collection/niar/'
    }
];

// --- backpack-type ---
export const typeItemsBP = [
    { text: 'Business Backpacks', href: '/backpacks/business/' },
    { text: 'Casual Backpacks', href: '/backpacks/casual-backpacks/' },
    { text: 'For Her', href: '/backpacks/for-her/' },
    { text: 'For Kids', href: '/backpacks/for-kids/' },
    { text: 'Shop all backpacks', href: 'https://ssmy.stg.samsonite-asia.com/backpacks/' }
];

// --- backpack-color ---
export const colorItemsBP = [
    { text: 'Mono', href: 'https://ssmy.stg.samsonite-asia.com/backpacks/black_grey_silver_white' },
    { text: 'Cool', href: 'https://ssmy.stg.samsonite-asia.com/backpacks/blue_green_navy_purple' },
    { text: 'Warm', href: 'https://ssmy.stg.samsonite-asia.com/backpacks/beige_orange_pink_red_yellow' },
    { text: 'Special', href: '/backpack/special/' },
    { text: 'Shop all colours', href: '/backpack/all-color/' }
];

// --- backpack-smart-function ---
export const smartFunctionItemsBP = [
    { text: 'USB Port', href: '/backpack/usb-port/' }
];

// --- backpack-brand ---
export const brandItemsBP = [
    { text: 'Samsonite', href: '/backpack/brand-samsonite/' },
    { text: 'Samsonite Black', href: '/backpack/brand-samsonite-black/' },
    { text: 'Samsonite Red', href: '/backpack/brand-samsonite-red/' }
];

// --- backpack-laptop ---
export const laptopItemsBP = [
    { text: 'Fit up to 13" laptop', href: '/backpacks/fit-up-to-13%22-laptop/' },
    { text: 'Fit up to 15" laptop', href: '/backpacks/fit-up-to-15%22-laptop/' },
    { text: 'Fit up to 17" laptop', href: '/backpacks/fit-up-to-17%22-laptop/' }
];

// --- backpack-collection ---
export const collectionItemsBP = [
    { text: 'Pro-Dlx 6', href: '/backpacks/collection/pro-dlx-6/' },
    { text: 'Prudence Eco', href: '/backpacks/collection/prudence-eco/' },
    { text: 'Sefton', href: '/backpacks/collection/sefton/' },
    { text: 'Locus Eco', href: '/backpacks/collection/locus-eco/' },
];

// --- bag-type ---
export const typeItemsB = [
    { text: 'Briefcase', href: '/bags/briefcase/' },
    { text: 'Cross Body Bags', href: '/bags/cross-body-bags/' },
    { text: 'Sling', href: '/bags/sling/' },
    { text: 'Totes', href: '/bags/totes/' },
    { text: 'For Her', href: '/bags/for-her/' },
    { text: 'Shop all bags', href: 'https://ssmy.stg.samsonite-asia.com/bags/' }
];

// --- bag-color ---
export const colorItemsB = [
    { text: 'Mono', href: 'https://ssmy.stg.samsonite-asia.com/bags/black_grey_white' },
    { text: 'Cool', href: 'https://ssmy.stg.samsonite-asia.com/bags/blue_green_navy' },
    { text: 'Warm', href: 'https://ssmy.stg.samsonite-asia.com/bags/beige_orange_pink_red_yellow' },
    { text: 'Shop all colours', href: '/bags/bags-all-color/' }
];

// --- bag-brand ---
export const brandItemsB = [
    { text: 'Samsonite', href: '/bags/brand-samsonite/' },
    { text: 'Samsonite Black', href: '/bags/brand-samsonite-black/' },
    { text: 'Samsonite Red', href: '/bags/brand-samsonite-red/' }
];

// --- bag-laptop ---
export const laptopItemsB = [
    { text: 'Fit up to 13" laptop', href: '/bags/fit-up-to-13%22-laptop/' },
    { text: 'Fit up to 15" laptop', href: '/bags/fit-up-to-15%22-laptop/' },
    { text: 'Fit up to 17" laptop', href: '/bags/fit-up-to-17%22-laptop/' }
];

// --- bag-collection ---
export const collectionItemsB = [
    { text: 'Abbey', href: '/bags/collection/abbey/' },
    { text: 'Cadiaz', href: '/bags/collection/cadiaz/' },
    { text: 'Sefton', href: '/bags/collection/sefton/' },
    { text: 'Zalia 3', href: '/bags/collection/zalia-3/' },
];

export const brandItemsBR = [
    { text: 'Samsonite BLACK LABEL', href: 'https://ssmy.stg.samsonite-asia.com/black/' },
    { text: 'Samsonite', href: 'https://ssmy.stg.samsonite-asia.com/samsonite/' },
    { text: 'Samsonite RED', href: 'https://ssmy.stg.samsonite-asia.com/red/' }
];

// --- discover-article ---
export const articleItems = [
    { text: `Mother's Day Gifts`, href: 'https://ssmy.stg.samsonite-asia.com/mothers-day-gifts.html' },
    { text: 'Your Business Look', href: 'https://ssmy.stg.samsonite-asia.com/your-business-look.html' },
    { text: 'Your Backpack Look', href: 'https://ssmy.stg.samsonite-asia.com/your-backpack-look.html' },
    { text: 'Lavish Travels', href: 'https://ssmy.stg.samsonite-asia.com/lavish-travels.html' },
    { text: 'Discover All', href: 'https://ssmy.stg.samsonite-asia.com/discover-all' }
];

// --- discover-about-Samsonite ---
export const aboutSamsoniteItems = [
    { text: 'Beyond The Design', href: 'https://ssmy.stg.samsonite-asia.com/2025-going-beyond-design/' },
    { text: 'Beyond The Average Test', href: 'https://ssmy.stg.samsonite-asia.com/2025-going-beyond-testing/' },
    { text: 'Our Brand Story - Celebrating 115 Years of Heritage', href: 'https://ssmy.stg.samsonite-asia.com/brand-story.html' },
    { text: 'Friends of Samsonite', href: 'https://ssmy.stg.samsonite-asia.com/2021-friends-of-samsonite.html' },
    { text: 'Our Responsible Journey', href: 'https://ssmy.stg.samsonite-asia.com/sustainability.html' }
];

// --- discover-collections ---
export const aboutCollections = [
    { text: 'Browse Our Collections', href: 'https://ssmy.stg.samsonite-asia.com/collection/' }
];

// --- offers-Refer a friend ---
export const referAFriend = [
    {
        hasImage: true,
        href: "https://ssmy.stg.samsonite-asia.com/referralshow",
        text: "Refer a Friend"
    }
]

// --- offers-Top10Picks ---
export const top10picks = [
    {
        hasImage: false,
        href: "https://ssmy.stg.samsonite-asia.com/2025-top-10-picks/",
        text: "Top 10 Picks"
    }
]

// --- accessories-type ---
export const typeItemAS = [
    { text: 'Travel accessories', href: '/accessories/travel-accessories/' },
    { text: 'Shop all accessories', href: 'https://ssmy.stg.samsonite-asia.com/accessories/' },
]

// --- accessories-color ---
export const colorItemAS = [
    { text: 'Mono', href: 'https://ssmy.stg.samsonite-asia.com/accessories/black_grey_white' },
    { text: 'Cool', href: 'https://ssmy.stg.samsonite-asia.com/accessories/blue_green_navy' },
    { text: 'Warm', href: 'https://ssmy.stg.samsonite-asia.com/accessories/beige_orange_pink_red_yellow' },
    { text: 'Special', href: '/accessories/special/' },
    { text: 'Shop all colours', href: '/accessories/accessories-all-color/' }
]



export const subMenuDataSamsonite = [
    {
        hasImage: true,
        href: "https://ssmy.stg.samsonite-asia.com/luggage/?prefn1=brand&prefv1=Samsonite",
        text: "Luggage"
    },
    {
        hasImage: true,
        href: "https://ssmy.stg.samsonite-asia.com/backpacks/?prefn1=brand&prefv1=Samsonite",
        text: "Backpacks"
    },
    {
        hasImage: true,
        href: "https://ssmy.stg.samsonite-asia.com/bags/?prefn1=brand&prefv1=Samsonite",
        text: "Bags"
    }
]

export const subMenuDataSamsoniteBlack = [
    {
        hasImage: true,
        href: "https://ssmy.stg.samsonite-asia.com/luggage/?prefn1=brand&prefv1=Samsonite%20Black%20Label",
        text: "Luggage"
    },
    {
        hasImage: true,
        href: "https://ssmy.stg.samsonite-asia.com/backpacks/?prefn1=brand&prefv1=Samsonite%20Black%20Label",
        text: "Backpacks"
    },
    {
        hasImage: true,
        href: "https://ssmy.stg.samsonite-asia.com/bags/?prefn1=brand&prefv1=Samsonite%20Black%20Label",
        text: "BRIEFCASES"
    }  
]

export const subMenuDataSamsoniteRed = [
    {
        hasImage: true,
        href: "https://ssmy.stg.samsonite-asia.com/luggage/?prefn1=brand&prefv1=Samsonite%20Red",
        text: "Luggage"
    },
    {
        hasImage: true,
        href: "https://ssmy.stg.samsonite-asia.com/backpacks/?prefn1=brand&prefv1=Samsonite%20Red",
        text: "Backpacks"
    },
    {
        hasImage: true,
        href: "https://ssmy.stg.samsonite-asia.com/bags/?prefn1=brand&prefv1=Samsonite%20Red",
        text: "Bags"
    }
]

export const checkoutDataWithoutFirstName = {
    lastName: "globeeLastName",
    email: "globeetest@mailinator.com",
    phone: "83298647",
    newsletter: true,
    terms: true
};

export const checkoutDataWithoutLastName = {
    firstName: "globeeFirstName",
    email: "globeetest@mailinator.com",
    phone: "83298647",
    newsletter: true,
    terms: true
};

export const checkoutDataWithoutEmail = {
    firstName: "globeeFirstName",
    lastName: "globeeLastName",
    phone: "83298647",
    newsletter: true,
    terms: true
};

export const checkoutDataWithoutPhone = {
    firstName: "globeeFirstName",
    lastName: "globeeLastName",
    email: "globeetest@mailinator.com",
    newsletter: true,
    terms: true
};

export const checkoutFullData = {
    firstName: "globeeFirstName",
    lastName: "globeeLastName",
    email: "globeetest@mailinator.com",
    phone: "83298647",
    newsletter: true,
    terms: true
};

export const checkoutShippingData = {
    postcode: "951111",
    address1: "65 XASSSERRR"
}